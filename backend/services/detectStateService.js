      ...
      if (updated) {
        ...
        if (row.active_scan_id) {
          try {
            ...
            if (rpcErr) {
              console.warn("[detectState] FAILED finalization: compute_scan_risk RPC failed, using fallback", rpcErr);
              const { computeRiskScore } = require("../utils/scoring");
              score = computeRiskScore([]);
            } else {
              score = finalScore ?? 0;
              console.log(`[detectState] FAILED finalization: compute_scan_risk returned ${score} for scan ${row.active_scan_id}`);
            }

            // Update network risk with final score
            // Note: active_scan_id is bigint (from scans table), don't pass as scanId
            // since networks.last_scan_id is uuid (from vulnerability_scans table)
