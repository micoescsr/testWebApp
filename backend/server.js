...
async function persistThreatRows(threatRows, scanId, activeNetworkId, supabaseClient) {
  ...
  if (rpcErr) {
    console.error("[persistThreatRows/legacy] compute_scan_risk RPC failed", rpcErr);
  } else {
    console.log(`[persistThreatRows/legacy] compute_scan_risk returned ${rpcScore} for scan ${scanId}`);
  }
