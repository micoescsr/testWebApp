...
async function persistThreatRows(threatRows, scanId, activeNetworkId) {
  ...
  // Recompute risk score via authoritative DB RPC (never write scans.risk_score from JS)
  ...
  if (rpcErr) {
    console.error("[persistThreatRows] compute_scan_risk RPC failed", rpcErr);
  } else {
    console.log(`[persistThreatRows] compute_scan_risk returned ${rpcScore} for scan ${scanId}`);
  }

  // Risk pipeline: update network risk with real RPC score + official bucket
  if (activeNetworkId) {
    try {
      const { bucketize, updateNetworkRisk } = require("../utils/riskPipeline");
      const newScore = rpcScore ?? 0;
  ...
  try {
    ...
    if (row.active_scan_id) {
      try {
        ...
        console.log(`[stopDetection] Final compute_scan_risk returned ${finalScore} for scan ${row.active_scan_id}`);
