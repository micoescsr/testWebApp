...
- Manual QA checklist added: `docs/TESTING_AND_QUALITY_ASSURANCE.md` §8.
- Not yet run: the manual checklist itself, and the Supabase Dashboard pre-flight checks (TOTP toggle enabled, `aal` claim present in issued JWTs) — both require live Supabase project access outside this session; see `docs/AUTHENTICATION_AND_AUTHORIZATION.md`'s "Needs Verification" section.
