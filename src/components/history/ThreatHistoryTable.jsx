// components/history/ThreatHistoryTable.jsx
import SortHeader from "./SortHeader";

const ThreatHistoryTable = ({
  data,
  onView,
  sortField,
  sortDir,
  onSort,
  loading,
  emptyMessage = "No threat scan history found.",
  footer,
}) => {
  const headerProps = { sortField, sortDir, onSort };

  return (
    <div className="history-card">
      <div className="history-card-header">
        <h3>Scan History — Threats</h3>
      </div>

      <div className="history-table-scroll">
        <table className="history-table history-table-summary">
          <thead>
            <tr>
              <SortHeader field="datetime" label="DATE & TIME" {...headerProps} />
              <SortHeader field="network" label="NETWORK" {...headerProps} />
              <SortHeader field="risk" label="RISK SCORE" {...headerProps} />
              <SortHeader field="vulns" label="VULNS" {...headerProps} />
              <SortHeader field="threats" label="THREATS" {...headerProps} />
              <th className="th-right">ACTION</th>
            </tr>
          </thead>

          <tbody>
            {loading ? (
              <tr>
                <td colSpan={6} className="history-empty-cell">
                  Loading scan history…
                </td>
              </tr>
            ) : data.length === 0 ? (
              <tr>
                <td colSpan={6} className="history-empty-cell">
                  {emptyMessage}
                </td>
              </tr>
            ) : (
              data.map((item) => (
                <tr key={item.id} className="history-row">
                  <td>{item.datetime}</td>
                  <td>{item.ssid || "—"}</td>
                  <td>
                    <span className={`risk-score-chip ${item.riskLevel}`}>
                      <span className="risk-score-dot" />
                      {item.riskScore} {item.riskLabel}
                    </span>
                  </td>
                  <td>{item.vulnCount}</td>
                  <td>{item.threatCount}</td>
                  <td className="td-right">
                    <button className="view-btn" onClick={() => onView(item)}>
                      VIEW <span className="view-btn-arrow">▶</span>
                    </button>
                  </td>
                </tr>
              ))
            )}
          </tbody>
        </table>
      </div>

      {footer && <div className="history-card-footer">{footer}</div>}
    </div>
  );
};

export default ThreatHistoryTable;
